# The ingest layer — fast job-listing ingest benchmark

> Deep-dive documentation for the ingest/benchmark layer. For the
> project overview, setup, and the recommendation engine, start at the
...
Arrow table of this run's rows
        │  ingest_and_benchmark.py (uv run; deps from pyproject.toml)
...
Requirements: `uv` — the project env comes from `pyproject.toml` +
`uv.lock` (`uv run` syncs it automatically; `requirements.txt` exists for
non-uv users). A Rust toolchain (MSVC on Windows; rusqlite's bundled
SQLite needs a C compiler, which MSVC provides) is only needed for
`--engine rust`.
