...
def _search_queries(con) -> list[str]:
    ...
    if len(tools) < 2:
        return ["engineer"]
    return [f"{tools[i]} {tools[(i + 3) % len(tools)]}" for i in range(len(tools))]
...
def bench_queries(db: Path, iters: int, k: int) -> None:
    ...
    cats = [
        ...
        for (c,) in con.execute(
            "SELECT DISTINCT job_category FROM jobs WHERE job_category IS NOT NULL"
        )
    ]
    rids = [
        r
    ...
    rows = [
        (
            ...
            lambda: recommend.filter_jobs(
                con,
                recommend.Filters(categories=[rotate(cats)], min_comp=100_000),
                limit=k,
            ),
        ),
        (
        ...
        (
            "keywords + filters",
            lambda: recommend.search_keywords(con, rotate(queries), flt, k=k),
        ),
        ("similar (knn)", lambda: recommend.similar_jobs(con, rotate(rids), k=k)),
        (
            "similar + filters",
...
def bench_build(db: Path, model: str) -> None:
    ...
    print(
        f"  full build of {s['rows']} rows: fts {s['fts_sec']:.2f}s, "
    ...
    s = recindex.build_index(db, model=model, quiet=True)
    total = s["diff_sec"] + s["fts_sec"] + s["embed_sec"]
    print(f"  incremental, 50 changed rows: {total:.2f}s ({s['changed']} reindexed)")
...
def bench_duckdb_appendix(db: Path, iters: int, k: int) -> None:
    ...
    con.execute(
        f"CREATE TABLE jobs AS SELECT requisition_id, title, "
    ...
    try:
        ...
        print(
            f"fts extension unavailable ({str(e).splitlines()[0]}); "
            f"skipping keyword comparison"
        )
        return
    t0 = time.perf_counter()
    con.execute(
        "PRAGMA create_fts_index('jobs', 'requisition_id', 'title', 'description')"
    )
    print(
        f"fts index build: {time.perf_counter() - t0:.2f}s "
...
def main() -> None:
    ...
    parser.add_argument(
        "--db", type=Path, help="existing (ingested + indexed) jobs.sqlite"
    )
    parser.add_argument(
        "--synthetic",
        type=int,
    ...
    parser.add_argument(
        "--skip-build", action="store_true", help="only run the query benchmarks"
    )
