...
def test_model_change_forces_rebuild(tmp_path):
    ...
    assert dict(sqlite3.connect(db).execute(
        "SELECT key, value FROM rec_meta"))["dim"] == "64"
...
def test_filter_category_and_comp(con):
    rows = recommend.filter_jobs(
        con, Filters(categories=["Healthcare"]), limit=50)
    ...
    assert rows and all(
        (r["yearly_max_compensation"] or 0) >= 150_000 for r in rows)


def test_filter_excludes_expired_by_default(con):
    assert not any(r["is_expired"]
                   for r in recommend.filter_jobs(con, limit=200))
    rows = recommend.filter_jobs(
        con, Filters(include_expired=True), limit=200)
    assert any(r["is_expired"] for r in rows)


def test_filter_tools_and_location(con):
    rows = recommend.filter_jobs(
        con, Filters(tools=["PostgreSQL"], location="San Francisco"),
        limit=50)
    assert rows
    for r in rows:
...
def test_incremental_change_and_delete(tmp_path):
    ...
    page["pageProps"]["job"]["job_information"]["title"] = \
        "Zorbulon Wrangler"
    ...
    with w:
        w.execute("DELETE FROM jobs WHERE requisition_id = ?",
                  ("SYN-nurse-00005",))
    w.close()
    stats = recindex.build_index(db, model="hash", quiet=True)
    assert stats["removed"] == 1
    con = recommend.get_connection(db)
    hits = {r["requisition_id"]
            for r in recommend.search_keywords(con, "nurse", k=50)}
...
def test_numpy_fallback_matches_vec0(tmp_path, monkeypatch):
    ...
    recindex.build_index(db, model="hash", quiet=True)
    con = recommend.get_connection(db)
    with_vec0 = [r["requisition_id"]
                 for r in recommend.similar_jobs(con, "SYN-mleng-00003",
                                                 k=5)]
    con.close()
...
def test_api_endpoints(corpus):
    ...
    assert r.status_code == 200
    assert {archetype_of(j["requisition_id"])
            for j in r.json()} == {"datasci"}
